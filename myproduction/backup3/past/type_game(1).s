...
TYPE_GAME_PRINT_TXT:
    ...
    move.l #SYSCALL_NUM_PUTSTRING, %d0
	move.l #0, %d1			 ** ch = 0
	move.l (TXT_P), %d2		 ** p = #TXT_P
	move.l	(TXT_SIZE), %d3	 ** size = (TXT_SIZE)
	trap #0
	movem.l (%SP)+, %d0-%d7
	rts
