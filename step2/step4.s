...
boot:
		...
		*move.l %a0, 0x110 /* STEP2.1 level 4, (64+4)*4 割り込み処理ルーチンの開始アドレスをレベル4割り込みベクタに設定 */
		...
		move.w #0xE10C, USTCNT1 |受信割り込み許可、送信割り込み無効 /* STEP2.2 */
...
INTERPUT:
	/*一回目だけ描画*/
	*move.b #0x65, %d0 /* URX1の下位8ビットのデータを転送 */
	*addi.w #0x0800, %d0 /* 即値をレジスタd0に加算 *
	*move.w %d0, UTX1 /* 16ビットのデータをUTX1に転送 */
	...
	cmp.b #0x00, %d0		/*step4.4:戻り値が0で分岐*/
	beq INTERPUT_MASK
...
INTERPUT_MASK:
	move.l #0xE108, USTCNT1	/*step4.4:送信割り込みのマスク*/
